...
const StakeSuccessModal = () => {
    const { chain } = useNetwork()
    const chainId: number = chain?.id ?? 56
	const { openStakeSuccessModal: open, setOpenStakeSuccessModal, stakeHash }: any = useContext(RefContext)
	const close = async () => setOpenStakeSuccessModal(false)
	return (
		<Modal open={open}>
			<div className={'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 outline-none w-[calc(100vw-40px)] md:w-[500px] bg-white rounded-[32px] p-[43px] relative'}>
				...
				<div className='flex flex-col items-center text-center'>
					...
					<span className='text-[23px] mt-16'>You have staked successfully!</span>
					<a className='text-pink1 text-[17px] mt-5' target='_blank' href={`https://${info[chainId]?.EXPLORER}/tx/${stakeHash}`}>View on {info[chainId]?.EXPLORER}</a>
				</div>
			</div>
		</Modal>
	)
